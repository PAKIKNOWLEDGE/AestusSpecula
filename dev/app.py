...
# ---- PWA static files (catch-all for dev mode) ---------------------------
...
async def pwa_static(path: str):
    if path.startswith("app/") or path.startswith("channel/") or path.startswith("healthz"):
        raise HTTPException(404)
    safe = path if path and path != "" else "index.html"
    for bad in ("..", "~", "//"):
        if bad in safe:
    ...
    if file.is_file():
        ...
        return FileResponse(str(file), media_type=media)
